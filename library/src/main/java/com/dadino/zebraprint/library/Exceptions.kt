...
class ActivityNotSetException() : PrintLibraryException(R.string.error_printer_generic.asFormattable(), message = "Activity not set in ZebraPrint. Remember to call zebraPrint.setActivity(activity) before using ZebraPrint APIs")
...
open class PrintLibraryException(val contextFormattable: ContextFormattable, message: String? = null) : RuntimeException(message)